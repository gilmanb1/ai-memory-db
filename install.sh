...
mkdir -p "$HOOKS_DIR"
for hook in pre_compact session_start session_end user_prompt_submit status_line _extract_worker remember_cmd forget_cmd reflect_cmd memory_mcp_server post_tool_use knowledge_cmd recalled_cmd audit_cmd health_cmd help_cmd; do
...
for cmd in remember forget forget-confirm memories facts search-memory decisions entities relationships sessions scopes reflect knowledge recalled session-learned review export-memory import-memory restore-memory snapshots audit-memory memory-health memory-help; do
